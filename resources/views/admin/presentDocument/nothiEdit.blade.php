...
@if(Auth::guard('admin')->user()->id == $checkDesiId->admin_id)
                                      <input disabled type="checkbox" data-nid="{{ $nothiLists->id }}" class="passBranch1" data-bId="{{ $allTotalBranchList->id }}" data-status="desi" value="{{ $allDesiList->id }}" name="designation[]" id="designation{{ $allDesiList->id }}">
                                      @else
