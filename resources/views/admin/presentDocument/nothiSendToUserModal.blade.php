...
->where('id',$finalAdminLists->designation_list_id)->value('designation_name');
                                                    ...
                                                    <td>
                                                        <div class="d-flex justify-content-center">
                                                            <div class="custom_checkbox">
                                                                <input id="ccheck{{ $finalAdminLists->id }}" value="{{ $finalAdminLists->id }}" name="nothiPermissionId" class="custom_check chb" type="checkbox">
                                            ...
                                            $adminSenderList = DB::table('nothi_details')
...
@foreach($adminSenderList as $adminSenderLists)

<?php
$finalAdminList = DB::table('admins')->where('id',$adminSenderLists->receiver)
                                                                        ->value('admin_name_ban');
$finalAdminDesignation = DB::table('admins')->where('id',$adminSenderLists->receiver)
                                                    ->value('designation_list_id');

$finalAdminBranch = DB::table('admins')->where('id',$adminSenderLists->receiver)
                                                    ->value('branch_id');

$designationName = DB::table('designation_lists')->where('id',$finalAdminDesignation)
                   ->value('designation_name');

 $branchName = DB::table('branches')->where('id',$finalAdminBranch)
                   ->value('branch_name');
...
?>
                                                ...
                                                <tr>
                                                    ...
                                                    <td style="width:80%">
                                                        {{ $finalAdminList }},
                                                        {{ $designationName }},{{ $branchName }}, এঞ্জিও
                                                        বিশয়ক ব্যুরো
                                                    </td>
                                                    <td>
                                                        <span style="color: #7e8ba8"><i class="fa fa-check-square-o"></i> স্বাক্ষরকারী ব্যাক্তি</span>
